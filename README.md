...
Runs in GitHub Actions so your PC does not need to stay on. It starts well before the 9:00 AM Eastern release window, waits until 9:00 inside the job, then polls Resy for about two minutes. If GitHub starts the scheduled job late, it still runs and emails diagnostics instead of silently skipping.
...
- `10:07 UTC`, which is 6:07 AM Eastern during daylight saving time.
- `11:07 UTC`, which is 6:07 AM Eastern during standard time.

A guard step skips the wrong-season duplicate. The real run stays dormant until 9:00 AM Eastern if GitHub starts it early, then performs 30 checks, 4 seconds apart. If GitHub starts it late, it checks immediately and the email marks `Late schedule start: true`. Each email includes the actual Eastern start time so you can see whether GitHub Actions queued the job late.
