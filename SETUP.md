...
The workflow is scheduled for 6:07 AM Eastern, then stays dormant inside the job until 9:00 AM Eastern before checking Resy. It has two UTC cron entries so it still works when New York switches between daylight saving time and standard time. A guard step prevents duplicate scheduled runs, and each email shows the actual Eastern start time. If GitHub starts the scheduled job late, it still checks immediately and marks `Late schedule start: true` in the email.
